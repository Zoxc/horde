...
impl<T> TableRef<T> {
    ...
    fn layout(bucket_count: usize) -> Option<(Layout, usize)> {
        ...
        // There can in theory be padding at the start of the layout
        // if the alignment of `T` is smaller than the larger alignment of
        // `TableInfo` and `Group`, but that doesn't currently occur.
